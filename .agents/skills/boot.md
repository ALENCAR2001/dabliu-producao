...
5b. **Addy Agent Skills (modo ativo).** Verify `vendor/agent-skills/skills/using-agent-skills/SKILL.md` exists.
   - If missing: `git clone --depth 1 https://github.com/addyosmani/agent-skills.git vendor/agent-skills`
   - Read `skills/addy-agent-skills.md` (integration with this kit). Do not load all 23 skills — only the meta-skill now; task-specific skills are read when dispatching.
   - Inform the user: *Agent Skills mode is ON* (Cursor rule + Maestro). See `docs/ADDY-AGENT-SKILLS.md`.
